...
valid_tag_ids = {
                  ...
                  3 : 165, # 8.5" x 11" tag black border size in mm
                }
...
clock = time.clock()
closeenoughrange = 10 # 10 to the left as buffer, and right.  20 total leeway
while(True):
    clock.tick()
    img = sensor.snapshot()
    ...
    if tags and (tags[0].id() in valid_tag_ids):
        ...
        nope = 0
        print(("Distance %f mm %d ; %d ; %d ; %d" % (z_to_mm(tags[0].z_translation(), valid_tag_ids[tags[0].id()]), tags[0].w(), tags[0].h(), tags[0].cx(), tags[0].cy())))
        if (tags[0].cx()< 80- closeenoughrange):
            print("Move left")
            nope = 1
        else:
            if (tags[0].cx()> 80+ closeenoughrange):
                print("Move right")
                nope = 1
        if (tags[0].cy()< 60- closeenoughrange):
            print("Move forward")
            nope = 1
        else:
            if (tags[0].cy()> 60+ closeenoughrange):
        ...
        #as we get closer, we need a wider close range.

        #"Distance %f mm - FPS %f" % (z_to_mm(tags[0].z_translation(), valid_tag_ids[tags[0].id()]), clock.fps()))
   # else:
